...
DELETE_ELEMENTS = ['<\?xml.*?>','<st1:chmetcnv.*?>', '</st1:chmetcnv>', '<br[\w\W]*?>', '<a name="_GoBack"/>', '<chsdate[\w\W]*?>', '</chsdate>', '<p[\w\W]*?>', '<span[\w\W]*?>', '<b style=[\w\W]*?>', '</span>', '<div[\w\W]*?>', '</div>', '<br/>', '</b>', '<br clear="none"/>', '<center[\w\W]*?>', '<font[\w\W]*?>', '</font>', '</center>', '<o:p>', '</o:p>', '\xa0', '\t']
...
class Tomd:
    ...
    def delete_info(self):
        for index, element in enumerate(DELETE_ELEMENTS):
            self.html = re.sub(element, '', self.html)
        return self.html
